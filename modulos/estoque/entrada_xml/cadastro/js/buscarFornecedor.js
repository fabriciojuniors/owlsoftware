function buscarClie(cod){
    ...
    fetch("/owlsoftware/modulos/estoque/entrada_xml/cadastro/controller/bClienteE.php",{
        ...
        if(text == 'null'){
            alert("Não foi possível localizar fornecedor com o código informado.");
...
function pesquisarCli(){
    ...
    fetch("/owlsoftware/modulos/estoque/entrada_xml/cadastro/controller/bClienteModal.php",{
        ...
        if(text == 'null'){
            resultado.innerHTML = "Não foi possível localizar fornecedores cadastrados.";
        }else{
...
function pesquisarFornXML(fornecedor){
    $('#cpfcnpj').val(fornecedor);
    $('#cpfcnpj').mask('99.999.999/9999-99');
    var CPF_CNPJ = document.getElementById("cpfcnpj").value;
    var dados = {
        cpfcnpj: CPF_CNPJ
    }
    fetch("/owlsoftware/modulos/estoque/entrada_xml/cadastro/controller/bFornecedorXML.php",{
        method: "POST",
        body: JSON.stringify(dados),
        headers: {'Content-Type': 'application/json'}
    }).then(function(response){
        ...
        try{
            let fornecedor = JSON.parse(text);
            buscarClie(fornecedor[0][0]);
        }catch{
