...
function pesquisar(){
    ...
    if(movimento){
        ...
        fetch("/owlsoftware/modulos/financeiro/recebimento/liquidacao/controller/carregaORMovto.php",{
            method: "POST",
            ...
            if(text == 0){
                ...
                divResultado.innerHTML = "<table class=' table table-sm table-striped' id='tabelaResultado'><thead><th>N° OR/Parcela</th><th>Cliente</th><th>Valor Movimentação</th><th>Data Movimentação</th><th>Tipo movto. Financeiro</th><th>Excluir?</th></thead>";
        ...
        fetch("/owlsoftware/modulos/financeiro/recebimento/liquidacao/controller/carregaOR.php",{
            method: "POST",
            body: JSON.stringify(dados),
            headers: {'Content-type': 'Application/json'}
        }).then(function(response){
		...
		mes = (mes < 10) ? mes + 1 : mes;
                ...
                divResultado.innerHTML = "<table class=' table table-sm table-striped' id='tabelaResultado'><thead><th>N° OR/Parcela</th><th>Cliente</th><th>Vencimento</th><th>Valor</th><th>Valor Saldo</th><th>Valor Movimentação</th><th>Data Movimentação</th><th>Movimentar?</th></thead>";
                var tabela = document.getElementById("tabelaResultado");
...
function excluir(id){
    ...
    fetch("/owlsoftware/modulos/financeiro/recebimento/liquidacao/controller/excluirMovto.php",{
        method: "POST",
        body: JSON.stringify(dados),
        headers: {'Content-type': 'Application/json'}
    }).then(function(response){
        ...
        for(i = 1; i<= qtd ; i++){
            ...
            if(movimentar === true){
                ...
                if(conta == '#' || tpmovto == '#'){
                    ...
                    var orparcela = document.getElementById("or-parcela-"+parcelaID).innerText;
                    ...
                    fetch("/owlsoftware/modulos/financeiro/recebimento/liquidacao/controller/movimentaOR.php",{
                        method: "POST",
                        body: JSON.stringify(dados),
                        headers: {'Content-type': 'Application/json'}
                    }).then(function(response){
