<?php
    ...
    $dados = json_decode($json, true);
        ...
        $sqlNumOR = "SELECT MAX(numero) FROM ordem_recebimento";
        $queryNumOR = mysqli_query($link, $sqlNumOR) or die (mysqli_error($link));
        $linha = mysqli_fetch_array($queryNumOR);
        $numeroOR = $linha[0] + 1;

        //Faz o insert da OR
        $sqlOR = "INSERT INTO ordem_recebimento(numero, cliente, emissao, valor_total, valor_saldo, condicao, forma_pagamento,  observacao, usuario_cadastro, status) VALUES($numeroOR, $dados[cliente], '$dados[emissao]', $dados[valor],$dados[valor] , $dados[condicao], $dados[formapag], '$dados[obs]', '', 1)";
        $queryOR = mysqli_query($link, $sqlOR) or die(mysqli_error($link));
        if($sqlNumOR){
            $sqlIDOR = "SELECT MAX(id) FROM ordem_recebimento";
            ...
            $j = 1;  
            foreach ($data as $d) {
                    $sqlParcela = "INSERT INTO parcela_or(ordem_recebimento, numero, emissao, vencimento, valor, valor_saldo, status) VALUES($idOR, $j, '$dados[emissao]', '$d', $valorParcelas, $valorParcelas, 1)";
            ...
            echo "Erro ao salvar ordem de recebimento. \n" . mysqli_error($link);
        }
