...
$result_banco = "SELECT ordem_pagamento.numero, clientes.nome, ordem_pagamento.valor_total, ordem_pagamento.id, ordem_pagamento.emissao, status_op.descricao FROM ordem_pagamento INNER JOIN clientes INNER JOIN status_op where ordem_pagamento.status = status_op.id AND ordem_pagamento.emissao BETWEEN '$imessao' AND '$femissao' AND ordem_pagamento.cliente = clientes.id AND ordem_pagamento.cliente LIKE '%$data[cliente]%' AND ordem_pagamento.numero LIKE '%$data[nop]%' ORDER BY id desc LIMIT $inicio, $qnt_result_pg";
	...
	<table class="table table-hover table-sm">
		...
		<tbody>
			...
			while($row = mysqli_fetch_array($resultado_banco)){
					...
					<td><a onclick="editarOP( <?php echo $row[3] ?>)"><i class="fas fa-pen"></i></a></td>
					</tr>
					<?php
				}
				?>
...
$result_pg = "SELECT COUNT(id) AS num_result FROM $tabela WHERE cliente LIKE '%$data[cliente]%'  AND ordem_pagamento.numero LIKE '%$data[nop]%'";
