...
$pagina .='<p><strong>Fim emissão:</strong>'.$femissao.'</p>';
    ...
    if(mysqli_num_rows($query)>0){
         $pagina .="<table class='table table-sm' id='tabelaResultado'>";
            $pagina.="<thead>";
            $pagina.='<th  align="center">Origem Docto</th>';
            $pagina.='<th align="center">N° Docto</th>';
            $pagina.='<th>Cliente/Fornecedor</th>';
            $pagina.='<th align="center">Data</th>';
            $pagina.='<th align="center">Movimentação</th>';
        ...
        foreach ($resultado as $row) {
            $teste = $row[2] ." | " . $row[3];
            $pagina.="<tr>";
            $pagina.="<td align=center'>".$row[0]."</td>";
            $pagina.="<td align=center'>".$teste."</td>";
            $pagina.="<td>".$row[1]."</td>";
            $pagina.="<td>".implode("/", array_reverse(explode("-", $row[4])))."</td>";
            $pagina.="<td align='center'>".$row[6]."</td>";
            $pagina.="<td align='center'>".$row[7]."</td>";
            $pagina.="<td align='center'>R$".$row[5]."</td>";
            $pagina.="<td align='center' id='saldo'>R$";  
                        if($i == 0){
                        if($row[0] == "OR"){
                            $pagina.=$row[5];
                        }else{
...
header("Cache-Control: no-cache, musy-revalidate");
header("Pragma: no-cache");
header("Content-type: application/x-msexcel");
header("Content-Disposition: attachment; filename=movimentacaoBancaria.xls");
