...
function atualizar($dados, $link){
    $dataHoje = date("Y/m/d");  
    $sql = "UPDATE grupo_produto SET descricao = '$dados[descricao]', inativo = $dados[inativo], usuario_atualizacao = '$dados[usuario_atualizacao]', dt_atualizacao =  '$dataHoje', grupo_venda = $dados[venda] WHERE id = $dados[id]";
...
function cadastrar($dados, $link){
        $sql = "select max(CAST(cod as int)) from grupo_produto";
        $query = mysqli_query($link, $sql);
...
function salvar($link, $dados, $cod){
    ...
    $sql = "INSERT INTO grupo_produto(cod, descricao, dt_criacao, dt_atualizacao, usuario_atualizacao, usuario_criacao, inativo, grupo_venda) VALUES('$cod', '$dados[descricao]', '$dataHoje', '$dataHoje', '$dados[usuario_atualizacao]', '$dados[usuario_criacao]', $dados[inativo], $dados[venda])";
    $query = mysqli_query($link, $sql);
