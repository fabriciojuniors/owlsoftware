...
function atualizar($dados, $link){
    $dataHoje = date("Y/m/d");  
    $sql = "UPDATE forma_pagamento SET descricao = '$dados[descricao]', inativo = $dados[inativo], usuario_atualizacao = '$dados[usuario_atualizacao]', dt_atualizacao =  '$dataHoje', avista = $dados[avista] WHERE id = $dados[id]";
...
function cadastrar($dados, $link){
        $sql = "select max(CAST(cod as int)) from forma_pagamento";
        $query = mysqli_query($link, $sql);
...
function salvar($link, $dados, $cod){
    ...
    $sql = "INSERT INTO forma_pagamento(cod, descricao, dt_criacao, dt_atualizacao, usuario_atualizacao, usuario_criacao, inativo, avista) VALUES('$cod', '$dados[descricao]', '$dataHoje', '$dataHoje', '$dados[usuario_atualizacao]', '$dados[usuario_criacao]', $dados[inativo], $dados[avista])";
    $query = mysqli_query($link, $sql);
