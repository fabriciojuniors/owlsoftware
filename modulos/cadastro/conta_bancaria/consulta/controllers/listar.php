...
$result_banco = "SELECT * FROM $tabela ORDER BY cod ASC LIMIT $inicio, $qnt_result_pg";
		...
		<tbody>
			...
			while($row = mysqli_fetch_assoc($resultado_banco)){
				if ($row['inativo'] == 1){
					...
					<td><?php echo $row['agencia'] ?> / <?php echo $row['dv_agencia'] ?>  </td>
					<td><?php echo $row['conta'] ?> / <?php echo $row['dv_conta'] ?> </td>
					<td><?php echo $row['titular'] ?></td>
					<td><a onclick="editar(<?php echo $row['id']?>,<?php echo $row['cod']?>, <?php echo $row['banco'] ?>, '<?php echo $row['agencia'] ?>', '<?php echo $row['dv_agencia']?>', '<?php echo $row['conta']?>', '<?php echo $row['dv_conta']?>', <?php echo $row['inativo']?>, '<?php echo $row['dt_criacao']?>', '<?php echo $row['usuario_criacao']?>', '<?php echo $row['titular']?>' )"><i class="fas fa-pen"></i></a></td>
				</tr>
					<?php
				} else{
					?>
