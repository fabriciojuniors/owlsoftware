...
function atualizar($dados, $link){
    $dataHoje = date("Y/m/d");  
    $sql = "UPDATE tp_movto_estoque SET descricao = '$dados[descricao]', inativo = $dados[inativo], usuario_atualizacao = '$dados[usuario_atualizacao]', dt_atualizacao =  '$dataHoje', operador = '$dados[operador]' WHERE id = $dados[id]";
...
function cadastrar($dados, $link){
        $sql = "select max(CAST(cod as int)) from tp_movto_estoque";
        $query = mysqli_query($link, $sql);
...
function salvar($link, $dados, $cod){
    ...
    $sql = "INSERT INTO tp_movto_estoque(cod, descricao, dt_criacao, dt_atualizacao, usuario_atualizacao, usuario_criacao, inativo, operador) VALUES('$cod', '$dados[descricao]', '$dataHoje', '$dataHoje', '$dados[usuario_atualizacao]', '$dados[usuario_criacao]', $dados[inativo], '$dados[operador]')";
    $query = mysqli_query($link, $sql);
