...
function atualizar($dados, $link){
    $dataHoje = date("Y/m/d");  
    $sql = "UPDATE pais SET nome = '$dados[descricao]', inativo = $dados[inativo], usuario_atualizacao = '$dados[usuario_atualizacao]', dt_atualizacao =  '$dataHoje', sigla = '$dados[sigla]' WHERE id = $dados[id]";
...
function cadastrar($dados, $link){
    if($dados['cod'] != ''){
        $cod = $dados['cod'];
        $sql = "SELECT * FROM pais WHERE cod='$cod'";
        $query = mysqli_query($link, $sql);
        ...
        $sql = "select max(CAST(cod as int)) from pais";
        $query = mysqli_query($link, $sql);
...
function salvar($link, $dados, $cod){
    ...
    $sql = "INSERT INTO pais(cod, nome, dt_criacao, dt_atualizacao, usuario_atualizacao, usuario_criacao, inativo, sigla) VALUES('$cod', '$dados[descricao]', '$dataHoje', '$dataHoje', '$dados[usuario_atualizacao]', '$dados[usuario_criacao]', $dados[inativo], '$dados[sigla]')";
    $query = mysqli_query($link, $sql);
