...
$result_banco = "SELECT * FROM $tabela ORDER BY id_pais ASC LIMIT $inicio, $qnt_result_pg";
		...
		<tbody>
			...
			while($row = mysqli_fetch_assoc($resultado_banco)){
				if ($row['inativo'] == 1){
					...
					<td>
					<?php
						$sql = "SELECT nome from pais WHERE id = '$row[id_pais]'";
					...
					'<?php echo $row['usuario_criacao'] ?>' , '<?php echo $row['dt_criacao'] ?>', '<?php echo $row['sigla'] ?>' , '<?php echo $row['id_pais'] ?>' )"><i class="fas fa-pen"></i></a></td>
				</tr>
					<?php
				} else{
					?>
					<tr>
                    	...
                    	'<?php echo $row['nome'] ?>', <?php echo $row['inativo'] ?>, '<?php echo $row['usuario_criacao'] ?>'  , '<?php echo $row['dt_criacao'] ?>' , '<?php echo $row['sigla'] ?>' , '<?php echo $row['id_pais'] ?>')"><i class="fas fa-pen"></i></a></td>
					</tr>
					<?php
				}
				?>
