		...
		<tbody>
			...
			while($row = mysqli_fetch_assoc($resultado_banco)){
				if ($row['inativo'] == 1){
					...
					'<?php echo $row['usuario_criacao'] ?>' , '<?php echo $row['dt_criacao'] ?>', '<?php echo $row['origem_movto_estoque'] ?>' , '<?php echo $row['tp_movto_estoque'] ?>', '<?php echo $row['gera_financeiro'] ?>' , '<?php echo $row['devolucao'] ?>')"><i class="fas fa-pen"></i></a></td>
				</tr>
					<?php
				} else{
					?>
