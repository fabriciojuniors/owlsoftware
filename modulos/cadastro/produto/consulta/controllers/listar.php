...
$result_banco = "SELECT * FROM $tabela where descricao like '%$data[descricao]%' and cod like '%$data[cod]%' ORDER BY cod ASC LIMIT $inicio, $qnt_result_pg";
		...
		<tbody>
			...
			while($row = mysqli_fetch_assoc($resultado_banco)){
				if ($row['inativo'] == 1){
					...
					<td><a onclick="editar( <?php echo $row['id'] ?>)"><i class="fas fa-pen"></i></a></td>
				</tr>
					<?php
				} else{
					?>
