...
function atualizar($dados, $link){
    $dataHoje = date("Y/m/d");  
    $sql = "UPDATE banco SET nome = '$dados[descricao]', inativo = $dados[inativo], usuario_atualizacao = '$dados[usuario_atualizacao]', dt_atualizacao =  '$dataHoje' WHERE id = $dados[id]";
...
function cadastrar($dados, $link){
    if($dados['cod'] != ''){
        ...
        $sql = "select max(CAST(cod as int)) from BANCO";
        $query = mysqli_query($link, $sql);
...
function salvar($link, $dados, $cod){
    ...
    $sql = "INSERT INTO banco(cod, nome, dt_criacao, dt_atualizacao, usuario_atualizacao, usuario_criacao, inativo) VALUES('$cod', '$dados[descricao]', '$dataHoje', '$dataHoje', '$dados[usuario_atualizacao]', '$dados[usuario_criacao]', $dados[inativo])";
    $query = mysqli_query($link, $sql);
