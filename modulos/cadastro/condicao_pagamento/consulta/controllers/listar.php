...
$result_banco = "SELECT * FROM $tabela ORDER BY CAST(cod as int) DESC LIMIT $inicio, $qnt_result_pg";
		...
		<tbody>
			...
			while($row = mysqli_fetch_assoc($resultado_banco)){
				if ($row['inativo'] == 1){
                    ...
                    '<?php echo $row['usuario_criacao'] ?>' , '<?php echo $row['dt_criacao'] ?>', '<?php echo $row['parcela_minima'] ?>',
                    '<?php echo $row['parcela_maxima'] ?>', '<?php echo $row['condicao']?>')"><i class="fas fa-pen"></i></a></td>
				</tr>
					<?php
				} else{
					?>
					<tr>
                    	...
                    	'<?php echo $row['descricao'] ?>', <?php echo $row['inativo'] ?>, '<?php echo $row['usuario_criacao'] ?>'  , '<?php echo $row['dt_criacao'] ?>' , '<?php echo $row['parcela_minima'] ?>', '<?php echo $row['parcela_maxima'] ?>' , '<?php echo $row['condicao']?>')"><i class="fas fa-pen"></i></a></td>
					</tr>
					<?php
				}
				?>
